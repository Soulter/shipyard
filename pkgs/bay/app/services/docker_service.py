...
class DockerService:
    ...
    async def create_ship_container(self, ship: Ship, spec: Optional[ShipSpec] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if settings.docker_network and settings.docker_network in network_settings.get("Networks", {}):
                ip_address = network_settings["Networks"][settings.docker_network].get("IPAddress")
            else:
    ...
    async def get_container_logs(self, container_id: str) -> str:
        ...
        try:
            ...
            # Get logs
            logs_stream = container.log(stdout=True, stderr=True)
            logs = ""
            async for line in logs_stream:
                logs += line
    ...
    def _build_container_config(self, ship: Ship, spec: Optional[ShipSpec] = None) -> Dict[str, Any]:
        """Build container configuration for aiodocker"""
        # Host configuration for resource limits
        host_config = {
            "RestartPolicy": {"Name": "no"},
            "PortBindings": {
                "8080/tcp": [{"HostPort": ""}]  # Let Docker assign random port
            }
        }
    ...
    def _parse_memory_string(self, memory_str: str) -> int:
        ...
        if memory_str.endswith('k') or memory_str.endswith('kb'):
            return int(memory_str[:-1] if memory_str.endswith('k') else memory_str[:-2]) * 1024
        elif memory_str.endswith('m') or memory_str.endswith('mb'):
            return int(memory_str[:-1] if memory_str.endswith('m') else memory_str[:-2]) * 1024 * 1024
        elif memory_str.endswith('g') or memory_str.endswith('gb'):
            return int(memory_str[:-1] if memory_str.endswith('g') else memory_str[:-2]) * 1024 * 1024 * 1024
        else:
